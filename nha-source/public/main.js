...
import { initChat, setCurrentUrn } from "./chat.js";
import { snapshotToServer } from "./snapshot.js";
...
function setupSnapshotButton(viewer) {
    const btn = document.getElementById("snapshot");
    const statusEl = document.getElementById("snapshot-status");
    if (!btn) return;

    btn.onclick = async () => {
        const urn = document.getElementById("models")?.value || window.location.hash?.substring(1);
        if (!urn) {
            alert("No model selected.");
            return;
        }
        btn.setAttribute("disabled", "disabled");
        if (statusEl) statusEl.textContent = "Starting snapshot...";

        try {
            await snapshotToServer(viewer, urn, {
                batchSize: 200,
                onProgress: ({ done, total, phase }) => {
                    if (!statusEl) return;
                    const pct = total ? Math.round((done / total) * 100) : 0;
                    statusEl.textContent = `${phase || "Snapshot"}: ${done}/${total} (${pct}%)`;
                },
            });
            if (statusEl) statusEl.textContent = "✅ Snapshot complete. SQLite updated.";
        } catch (e) {
            console.error(e);
            if (statusEl) statusEl.textContent = `❌ Snapshot failed: ${e.message || e}`;
